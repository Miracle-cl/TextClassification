...
class SimpleGRU(nn.Module):
    def __init__(self, weights, embed_dim=300, batch_size=256,
                 hidden_size=256, n_layers=1, dropout=0.5, output_size=1):
    ...
    def forward(self, input_var, input_len, hidden=None):
        ...
        print("\tIn Model: input size", input_var.size(), "output size", outputs.size(), embeded.size())
        # In Model: input size torch.Size([250, 400]) output size torch.Size([250, 1])
        return outputs
...
def train_epoch(model, epoch, train_loader, test_loader, criterion, optimizer, clip=1., batch_sz=256):
    ...
    for i, batch in enumerate(train_loader, 1):
        ...
        src = src.cuda()
        tgt = torch.FloatTensor(tgt).cuda() # tgt without modified - cuda out of memory

        optimizer.zero_grad() # here same as model.zero_grad()

        outputs = model(src, src_lens)
        # print("Outside: input size", src.size(), "output_size", outputs.size())
        # Outside: input size torch.Size([500, 400]) output_size torch.Size([500, 1])
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_loader, 1):
            ...
            total += len(tgt)
            src = src.cuda()
            tgt = torch.FloatTensor(tgt).cuda()
            outputs = model(src, src_lens)
            loss = criterion(outputs, tgt.unsqueeze(1))
            eval_loss += loss.item()
...
def main(BATCH_SIZE = 100, MAX_LEN = 400):
