...
class HierarchialAttentionNetwork(nn.Module):
    ...
    def __init__(self, n_classes, vocab_size, emb_size, word_rnn_size, sentence_rnn_size, word_rnn_layers,
                 sentence_rnn_layers, word_att_size, sentence_att_size, dropout=0.5, embed_weights=None):
        """
        ...
        self.sentence_attention = SentenceAttention(vocab_size, emb_size, word_rnn_size, sentence_rnn_size,
                                                    ...
                                                    sentence_att_size, dropout, embed_weights)
    ...
    def forward(self, documents, sentences_per_document, words_per_sentence):
        ...
        document_embeddings, word_alphas, sentence_alphas = self.sentence_attention(documents, 
...
class SentenceAttention(nn.Module):
    ...
    def __init__(self, vocab_size, emb_size, word_rnn_size, sentence_rnn_size, word_rnn_layers, sentence_rnn_layers,
                 word_att_size, sentence_att_size, dropout, embed_weights):
        """
        :param vocab_size: number of words in the vocabulary of the model
        :param emb_size: size of word embeddings
        :param word_rnn_size: size of (bidirectional) word-level RNN
        ...
        self.word_attention = WordAttention(vocab_size, emb_size, word_rnn_size, word_rnn_layers, word_att_size, dropout, embed_weights)
    ...
    def forward(self, documents, sentences_per_document, words_per_sentence):
        ...
        # (n_sentences) ： drop padding sentence
        ...
        (sentences, _), _ = self.sentence_rnn(PackedSequence(sentences, bs))  # (n_sentences, 2 * sentence_rnn_size), (max(sent_lens))
        ...
        documents = documents * sentence_alphas.unsqueeze(2)  # (n_documents, max_doc_len_in_batch, 2 * sentence_rnn_size)
        documents = documents.sum(dim=1)  # (n_documents, 2 * sentence_rnn_size)

        # Also re-arrange word_alphas (SENTENCES -> DOCUMENTS)
        word_alphas, _ = pad_packed_sequence(PackedSequence(word_alphas, bs), batch_first=True)  # (n_documents, max_doc_len_in_batch, max_sent_len_in_batch)
...
class WordAttention(nn.Module):
    ...
    """
    def __init__(self, vocab_size, emb_size, word_rnn_size, word_rnn_layers, word_att_size, dropout, embed_weights):
        """
        :param vocab_size: number of words in the vocabulary of the model
        :param emb_size: size of word embeddings
        :param word_rnn_size: size of (bidirectional) word-level RNN
        ...
        if embed_weights is None:
            ...
            self.embeddings = nn.Embedding.from_pretrained(torch.FloatTensor(embed_weights), freeze=False)
